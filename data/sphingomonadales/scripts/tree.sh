...
WD=/home/bio.aau.dk/zs85xk/projects/epsSMASH/epsSMASH_paper/sphingomonadales
cd $WD

# variables
THREADS=$(nproc)
FULL_MSA=output/gtdbtk/align/gtdbtk.bac120.msa.fasta.gz
ODIR=output/tree
mkdir -p $ODIR
...
GENOMES=genomes
ls $GENOMES | grep '.fna' | sed 's/\.fna$//' > $WD/mags_to_screen.txt


echo -e 'RS_GCF_021378375.1\nRS_GCF_000215485.1' >> $WD/mags_to_screen.txt # add outgroup genomes

conda activate fxtract_v2.4
fxtract -H -f $WD/mags_to_screen.txt $FULL_MSA > $ODIR/MSA_sphingomonadales_tree_subset_for_IQtree.faa
conda deactivate
...
iqtree2 -s $MSA -m WAG+G -B 1000 -T AUTO --threads-max $THREADS -o RS_GCF_021378375.1,RS_GCF_000215485.1
conda deactivate